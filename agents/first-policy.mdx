...
- **`asset`** — bytes32 of the asset symbol. **Optional** — the API auto-resolves the per-shield literal when omitted. Each shield expects a specific value (`BTC` for FlashBTC, `ETH` for FlashETH, `USDT` for MicroDepeg, `USDC` for RateShock); sending the wrong one reverts with `InvalidAsset(bytes32)`.
